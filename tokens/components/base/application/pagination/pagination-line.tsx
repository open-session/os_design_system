...
export const PaginationLine = ({ framed, className, size = "md", ...props }: PaginationLineProps) => {
    ...
    return (
        <Pagination.Root {...devProps('PaginationLine')} {...props} className={cx("flex h-max w-max", sizes[size].root, framed && "rounded-full bg-alpha-white/90 backdrop-blur", className)}>
            <Pagination.Context>
                {({ pages }) =>
