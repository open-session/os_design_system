...
const PaginationRoot = ({ total, siblingCount = 1, page, onPageChange, children, style, className }: PaginationRootProps) => {
    ...
    return (
        <PaginationContext.Provider value={paginationContextValue}>
            <nav {...devProps('PaginationRoot')} aria-label="Pagination Navigation" style={style} className={className}>
                {children}
            </nav>
...
const Trigger: FC<TriggerProps> = ({ children, style, className, asChild = false, direction, ariaLabel }) => {
    ...
    return (
        <button {...devProps('Trigger')} aria-label={ariaLabel || defaultAriaLabel} onClick={handleClick} disabled={isDisabled} style={style} className={computedClassName}>
            {children}
        </button>
    );
};

const PaginationPrevTrigger: FC<Omit<TriggerProps, "direction">> = (props) => <Trigger {...devProps('PaginationPrevTrigger')} {...props} direction="prev" />;

const PaginationNextTrigger: FC<Omit<TriggerProps, "direction">> = (props) => <Trigger {...devProps('PaginationNextTrigger')} {...props} direction="next" />;
...
const PaginationEllipsis: FC<PaginationEllipsisProps> = ({ children, style, className }) => {
    ...
    return (
        <span {...devProps('PaginationEllipsis')} style={style} className={computedClassName} aria-hidden="true">
            {children}
        </span>
    );
};
