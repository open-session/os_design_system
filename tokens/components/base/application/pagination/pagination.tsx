...
const PaginationItem = ({ value, rounded, isCurrent }: { value: number; rounded?: boolean; isCurrent: boolean }) => {
    return (
        <Pagination.Item
            {...devProps('PaginationItem')}
            value={value}
            isCurrent={isCurrent}
            className={({ isSelected }) =>
                cx(
                    "flex size-9 cursor-pointer items-center justify-center p-3 text-sm font-medium text-quaternary outline-focus-ring transition duration-micro ease-linear hover:bg-primary_hover hover:text-secondary focus-visible:z-10 focus-visible:bg-primary_hover focus-visible:outline-2 focus-visible:outline-offset-2",
...
const MobilePagination = ({ page = 1, total = 10, className, onPageChange }: MobilePaginationProps) => {
    return (
        <nav {...devProps('MobilePagination')} aria-label="Pagination" className={cx("flex items-center justify-between md:hidden", className)}>
            <Button
...
export const PaginationPageDefault = ({ rounded, page = 1, total = 10, className, ...props }: PaginationProps) => {
    ...
    return (
        <Pagination.Root
            {...devProps('PaginationPageDefault')}
            {...props}
            page={page}
            total={total}
...
export const PaginationPageMinimalCenter = ({ rounded, page = 1, total = 10, className, ...props }: PaginationProps) => {
    ...
    return (
        <Pagination.Root
            {...devProps('PaginationPageMinimalCenter')}
            {...props}
            page={page}
            total={total}
            className={cx("flex w-full items-center justify-between gap-3 border-t border-secondary pt-4 md:pt-5", className)}
...
export const PaginationCardDefault = ({ rounded, page = 1, total = 10, ...props }: PaginationProps) => {
    ...
    return (
        <Pagination.Root
            {...devProps('PaginationCardDefault')}
            {...props}
            page={page}
            total={total}
...
export const PaginationCardMinimal = ({
    ...
    return (
        <div {...devProps('PaginationCardMinimal')} className={cx("border-t border-secondary px-4 py-3 md:px-6 md:pt-3 md:pb-4", className)}>
...
export const PaginationButtonGroup = ({ align = "left", page = 1, total = 10, ...props }: PaginationButtonGroupProps) => {
    ...
    return (
        <div
            {...devProps('PaginationButtonGroup')}
            className={cx(
...
export const PaginationCardAdvanced = ({
    ...
    return (
        <div {...devProps('PaginationCardAdvanced')} className={cx("border-t border-secondary px-4 py-3 md:px-6 md:pt-3 md:pb-4", className)}>
            <Pagination.Root
                page={page}
                total={total}
