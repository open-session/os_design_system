...
// eslint-disable-next-line bos-local/require-dev-props -- Context.Provider renders no DOM node; devProps has no root element to attribute
...
export const RangePresetButton = ({ value, className, children, ...props }: RangePresetButtonProps) => {
    ...
    return (
        <button
            {...devProps('RangePresetButton')}
            {...props}
            className={cx(
                "cursor-pointer rounded-md px-3 py-2 text-left text-sm font-medium outline-focus-ring transition duration-micro ease-linear focus-visible:outline-2 focus-visible:outline-offset-2",
