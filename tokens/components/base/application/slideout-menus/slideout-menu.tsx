...
import { CloseButton } from "@/components/base/base/buttons/close-button";
...
const Menu = ({ children, dialogClassName, ...props }: SlideoutMenuProps) => {
    return (
        <ModalOverlay {...devProps('Menu')} {...props}>
...
const Content = ({ role = "main", ...props }: ComponentPropsWithRef<"div">) => {
    return <div {...devProps('Content')} role={role} {...props} className={cx("flex size-full flex-col gap-6 overflow-y-auto overscroll-auto px-4 md:px-6", props.className)} />;
};
...
const Header = ({ className, children, onClose, ...props }: SlideoutHeaderProps) => {
    return (
        <header {...devProps('Header')} {...props} className={cx("relative z-1 w-full px-4 pt-6 md:px-6", className)}>
            {children}
...
const Footer = (props: ComponentPropsWithRef<"footer">) => {
    return <footer {...devProps('Footer')} {...props} className={cx("w-full p-4 shadow-[inset_0px_1px_0px_0px] shadow-border-secondary md:px-6", props.className)} />;
};
