...
import { FeaturedIcon as FeaturedIconbase } from "@/components/base/foundations/featured-icon/featured-icon";
import type { BackgroundPatternProps } from "@/components/base/shared-assets/background-patterns";
import { BackgroundPattern } from "@/components/base/shared-assets/background-patterns";
import { Illustration as Illustrations } from "@/components/base/shared-assets/illustrations";
...
const Root = ({ size = "lg", ...props }: RootProps) => {
    return (
        <RootContext.Provider value={{ size }}>
            <div {...devProps('Root')} {...props} className={cx("mx-auto flex w-full max-w-lg flex-col items-center justify-center", props.className)} />
...
const FeaturedIcon = ({ color = "gray", theme = "modern", icon = SearchLg, size = "lg", ...props }: ComponentPropsWithRef<typeof FeaturedIconbase>) => {
    ...
    return <FeaturedIconbase {...devProps('FeaturedIcon')} {...props} {...{ color, theme, icon }} size={rootSize === "lg" ? "xl" : size} />;
};
...
const FileTypeIcon = ({ type = "folder", theme = "solid", ...props }: FileTypeIconProps) => {
    return (
        <div {...devProps('FileTypeIcon')} {...props} className={cx("relative z-10 flex rounded-full bg-linear-to-b from-gray-50 to-gray-200 p-8", props.className)}>
...
const Footer = (props: ComponentPropsWithRef<"div">) => {
    return <footer {...devProps('Footer')} {...props} className={cx("z-10 flex gap-3", props.className)} />;
};
...
const Description = (props: ComponentPropsWithRef<"p">) => {
    ...
    return <p {...devProps('Description')} {...props} className={cx("text-center text-sm text-tertiary", size === "lg" && "text-md", props.className)} />;
};
