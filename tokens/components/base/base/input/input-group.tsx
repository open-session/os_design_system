...
import type { InputBaseProps } from "@/components/base/base/input/input";
import { TextField } from "@/components/base/base/input/input";
            ...
            {({ isDisabled, isInvalid, isRequired }) => (
                <>
                    ...
                    <div
                        ...
                        className={cx(
                            "group relative flex h-max w-full flex-row justify-center rounded-lg bg-primary transition-all duration-micro ease-motion-out",

                            // Only apply focus ring when child is select and input is focused
                            "has-[&>select]:shadow-xs has-[&>select]:ring-1 has-[&>select]:ring-border-primary has-[&>select]:ring-inset has-[&>select]:has-[input:focus]:ring-1 has-[&>select]:has-[input:focus]:ring-border-brand has-[&>select]:has-[input:focus]:shadow-focus-ring-elevated",
