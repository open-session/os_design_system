...
import { ArrowRight, BookOpen01 as BookOpen, Building01 as Building, Clock, File01 as FileText, Globe01, Home01 as Home, Image01 as ImageIcon, LayersTwo01 as Layers, LayoutGrid01 as LayoutGrid, LinkExternal01, Lock01, MessageSquare01 as MessageSquare, Palette, SearchLg, Type01 as Type, XClose, Zap, FaceId as ScanFace, CpuChip01 as BrainCog, Lightbulb02 as Lightbulb, Edit01 as Pencil, Cube01 as Boxes, MessagePlusSquare as MessageSquarePlus, Command } from '@untitledui-pro/icons/line';
...
function SearchResultSkeleton() {
  return (
    <div {...devProps('SearchResultSkeleton')} className="py-2">
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className="flex items-center gap-3 px-5 py-3">
          {/* Icon skeleton */}
          <div className="w-9 h-9 rounded-lg bg-bg-tertiary animate-pulse" />
          {/* Text skeleton */}
...
export function SearchModal({ isOpen, onClose }: SearchModalProps) {
  ...
  const handleSelect = useCallback((result: SearchResult) => {
    ...
    if (result.type === 'logo' || result.type === 'image' || result.type === 'illustration') {
      ...
      // Load the chat session via context and navigate to chat
  ...
  const renderPageResult = (result: PageResult, isSelected: boolean, globalIndex: number) => {
    ...
    return (
      ...
      >
        ...
        {isSelected && (
          <ArrowRight className="w-4 h-4 text-fg-tertiary flex-shrink-0" />
        )}
      </button>
    );
  };
    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-fg-tertiary truncate flex items-center gap-1">
  ...
  const renderAssetResult = (result: AssetResult, isSelected: boolean, globalIndex: number) => {
    ...
    return (
      ...
      >
        ...
        {isSelected && (
          <LinkExternal01 className="w-4 h-4 text-fg-tertiary flex-shrink-0" />
        )}
      </button>
    );
  };
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm font-medium text-fg-primary truncate flex items-center gap-1.5">
            {result.title}
            {result.isPrivate ? (
              <Lock01 className="w-3 h-3 text-fg-quaternary" />
            ) : (
              <Globe01 className="w-3 h-3 text-fg-quaternary" />
            )}
          </div>
          <div className="text-xs text-fg-tertiary truncate">
            {result.subtitle}
  ...
  const SectionHeader = ({ title, count }: { title: string; count?: number }) => (
    <div {...devProps('SectionHeader')} className="px-5 py-2 flex items-center justify-between">
      <span className="text-[10px] font-medium text-fg-quaternary uppercase tracking-wider">
        {title}
      </span>
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-3 px-5 py-4 border-b border-border-secondary">
                <SearchLg className="w-5 h-5 text-fg-tertiary flex-shrink-0" />
                <input
                  ref={inputRef}
                  type="text"
                  value={searchQuery}
                ...
                <div className="flex items-center gap-1">
                  <kbd className="hidden sm:flex items-center gap-0.5 px-1.5 py-0.5 text-[10px] text-fg-quaternary bg-bg-tertiary rounded border border-border-secondary">
              ...
              <div ref={resultsRef} className="max-h-[60vh] overflow-y-auto">
                {isInitialLoading ? (
                  ...
                  <div className="py-12 text-center">
                    <SearchLg className="w-8 h-8 text-fg-quaternary mx-auto mb-3" />
                    <p className="text-sm text-fg-tertiary">No results found</p>
                    <p className="text-xs text-fg-quaternary mt-1">Try a different search term</p>
                  </div>
                ) : (
                  <div className="py-8 text-center">
                    <p className="text-sm text-fg-tertiary">Start typing to search</p>
              ...
              {/* Footer */}
              <div className="flex items-center justify-between px-5 py-3 border-t border-border-secondary text-[10px] text-fg-quaternary">
                ...
                <span className="flex items-center gap-1">
                  <kbd className="px-1 py-0.5 bg-bg-tertiary rounded">esc</kbd>
                  to close
                </span>
              </div>
            </motion.div>
