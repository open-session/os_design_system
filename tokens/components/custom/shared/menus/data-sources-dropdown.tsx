...
import { Globe01, SearchLg } from '@untitledui-pro/icons/line';
// UUI fallback: Palette → SearchLg (no Palette equivalent; using SearchLg for Brand Search icon)
...
export function DataSourcesDropdown({
  ...
  const dataSources: DataSource[] = [
    ...
    {
      id: 'brand',
      icon: SearchLg, // UUI fallback: no Palette equivalent
      title: 'Brand Search',
  ...
  return (
    <div {...devProps('DataSourcesDropdown')} ref={dropdownRef} className="relative">
      <button
        ref={buttonRef}
        type="button"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute bottom-full mb-2 bg-bg-secondary rounded-xl border border-border-primary shadow-xl z-50"
            style={{
              ...
              {dataSources.map((source) => {
                ...
                return (
                  <div
                    ...
                    className="w-full flex items-start gap-3 p-3 rounded-lg hover:bg-bg-tertiary transition-colors group cursor-pointer"
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => {
                    ...
                    <div className="flex-shrink-0 mt-0.5">
                      <div
                        className={`
                          ...
                          ${source.enabled ? 'bg-bg-brand-solid' : 'bg-bg-tertiary'}
                        `}
