...
import { Globe01, SearchLg, ShoppingBag01 } from '@untitledui-pro/icons/line';
// UUI fallback: Orbit → Globe01 (no Orbit equivalent; Globe01 used for spatial/global context)
...
export function SearchResearchToggle({ 
  ...
  const CurrentIcon = activeMode === 'search' ? SearchLg : Globe01;
  ...
  return (
    <div ref={containerRef} {...devProps('SearchResearchToggle')} className="inline-block">
      ...
      <div className="relative inline-block">
        ...
        {shouldShowTooltip && (
          <div
            ...
            className="absolute top-full left-0 mt-3 w-80 max-w-[calc(100vw-2rem)] rounded-xl border border-border-secondary shadow-lg p-4 z-[10001] bg-bg-secondary"
          >
            ...
            <div className="relative">
              <h3 className="font-semibold text-fg-primary mb-1">
              ...
              <div className="border-t border-border-secondary pt-3">
                <p className="text-xs text-fg-brand-primary mb-2">
                ...
                <button className="w-full bg-bg-brand-solid hover:bg-bg-brand-solid-hover text-fg-white text-sm font-medium py-2 px-4 rounded-lg transition-colors">
                  Upgrade to Pro
                </button>
              </div>
            </div>
...
export function SearchResearchSuggestions({
  ...
  const CurrentIcon = mode === 'search' ? SearchLg : Globe01;
  ...
  return (
    <motion.div
      {...devProps('SearchResearchSuggestions')}
    ...
    >
      ...
      {isLoading && (
        <div className="flex items-center justify-center py-3">
          <div className="w-4 h-4 border border-fg-tertiary/30 border-t-fg-brand-primary rounded-full animate-spin" />
        </div>
      )}
          ...
          {suggestions.map((suggestion, index) => (
            ...
            >
              <div className="flex items-start space-x-3">
                <CurrentIcon className="w-4 h-4 text-fg-tertiary group-hover:text-fg-brand-primary mt-0.5 flex-shrink-0" />
                <span className="text-sm text-fg-primary group-hover:text-fg-brand-primary">
                  {suggestion}
                </span>
              </div>
            </motion.button>
