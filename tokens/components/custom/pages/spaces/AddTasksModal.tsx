...
export function AddTasksModal({
  ...
  return (
    <Modal {...devProps('AddTasksModal')} isOpen={isOpen} onClose={handleClose} title="Add Tasks" size="md">
      {/* Add new task form */}
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            <User01 className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-fg-tertiary" />
            <input
              id="task-assignee"
              type="text"
              value={assignee}
            ...
            {existingTasks.map((task) => (
              ...
              >
                <button
                  ...
                  className="mt-0.5 flex-shrink-0 text-fg-tertiary hover:text-fg-brand-primary transition-colors focus:outline-hidden focus:ring-1 focus:ring-brand focus:shadow-focus-ring rounded disabled:opacity-50 disabled:cursor-not-allowed"
                  aria-label={task.completed ? `Mark "${task.title}" as incomplete` : `Mark "${task.title}" as complete`}
                >
                  {task.completed ? (
                    <CheckSquare className="w-5 h-5 text-fg-brand-primary" />
                  ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {task.assignee && (
                    <div className="flex items-center gap-1 mt-1.5">
                      <User01 className="w-3 h-3 text-fg-tertiary" />
                      <span className="text-xs text-fg-tertiary">{task.assignee}</span>
                    </div>
                  )}
                </div>
                {onRemoveTask && (
                  <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-bg-error-primary text-fg-tertiary hover:text-fg-error-primary transition-colors flex-shrink-0 focus:outline-hidden focus:ring-1 focus:ring-brand focus:shadow-focus-ring-error"
