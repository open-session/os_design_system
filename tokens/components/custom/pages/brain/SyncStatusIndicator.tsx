...
// UUI fallback: Cloud → Server01 (no Cloud01 in UUI free/pro)
// UUI fallback: CloudOff → XCircle (no CloudOff in UUI)
...
export function SyncStatusIndicator({
  ...
  const getStatusDisplay = () => {
    switch (state.status) {
      case 'loading':
        return {
          icon: <RefreshCw01 className="h-4 w-4 animate-spin" />,
      ...
      default:
        return {
          icon: <Server01 className="h-4 w-4" />,
          color: 'text-fg-tertiary',
          label: 'Unknown',
        };
    }
  ...
  return (
    ...
    >
      ...
      <button
        ...
        title="Sync now"
      >
        <RefreshCw01 className={`h-4 w-4 ${isSyncing ? 'animate-spin' : ''}`} />
      </button>
