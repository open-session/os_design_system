...
// UUI fallback: MagicWand01 → Zap, ClipboardPlus → ClipboardCheck
...
export function AddDocumentModal({
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div
          {...devProps('AddDocumentModal')}
          initial={{ scale: 0.95, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          exit={{ scale: 0.95, opacity: 0 }}
          transition={{ duration: 0.2 }}
          onClick={(e) => e.stopPropagation()}
          className={`w-full ${mode === 'guided' ? 'max-w-3xl' : 'max-w-2xl'} max-h-[85vh] flex flex-col rounded-2xl bg-bg-primary border border-border-primary shadow-2xl overflow-hidden`}
        >
          {/* Header */}
    ...
    <div {...devProps('ModeSelector')} className="p-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <button
          ...
          className="group relative p-6 rounded-xl bg-bg-secondary border border-border-primary hover:border-border-brand hover:bg-secondary-hover transition-all text-left"
        >
          <div className="flex items-start justify-between mb-4">
            <div className="p-3 rounded-xl bg-bg-tertiary border border-border-primary">
              <ClipboardCheck className="w-6 h-6 text-fg-tertiary" />
            </div>
            <ArrowRight className="w-5 h-5 text-fg-tertiary opacity-0 group-hover:opacity-100 transition-opacity" />
          </div>
          <h3 className="text-lg font-display font-semibold text-fg-primary group-hover:text-fg-brand-primary transition-colors mb-2">
            Manual
          </h3>
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-3">
            <p className="text-sm text-fg-tertiary flex-1">
              Already have content? Paste it directly and we&apos;ll add it to your knowledge base.
            </p>
            <div className="text-xs text-fg-quaternary whitespace-nowrap sm:mt-0">
              Fastest option
            </div>
          </div>
        </button>
        ...
        >
          <div className="flex items-start justify-between mb-4">
            <div className="p-3 rounded-xl bg-bg-tertiary border border-border-primary">
              <Zap className="w-6 h-6 text-fg-tertiary" />
            </div>
            <ArrowRight className="w-5 h-5 text-fg-tertiary opacity-0 group-hover:opacity-100 transition-opacity" />
          </div>
          <h3 className="text-lg font-display font-semibold text-fg-primary group-hover:text-fg-brand-primary transition-colors mb-2">
...
function PasteMode({
  ...
  return (
    <div {...devProps('PasteMode')} className="p-6 space-y-4">
      {/* Title Input */}
      <div>
        <label htmlFor="doc-title" className="block text-sm font-medium text-fg-secondary mb-2">
          Document Title
        </label>
        <input
          id="doc-title"
      ...
      {/* Content Textarea */}
      <div>
        <label htmlFor="doc-content" className="block text-sm font-medium text-fg-secondary mb-2">
          Content (Markdown)
        </label>
        <textarea
          id="doc-content"
