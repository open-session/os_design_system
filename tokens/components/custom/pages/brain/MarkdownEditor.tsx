...
// UUI: Undo2 → FlipBackward, Redo2 → FlipForward, History → ClockFastForward, Code → Code01
        ...
        <div className="flex items-center gap-3">
          ...
          <AnimatePresence mode="wait">
            {isEditing && (
              <motion.span 
                ...
                className="px-2 py-0.5 text-xs font-medium rounded-md bg-bg-tertiary text-fg-secondary border border-border-secondary"
              >
                Editing
              </motion.span>
            )}
          ...
          {hasUnsavedChanges && !isEditing && (
            <button
              ...
              className="px-2 py-0.5 text-xs font-medium rounded-full bg-bg-warning-subtle text-fg-warning-primary hover:bg-bg-warning-secondary transition-colors flex items-center gap-1"
            >
              ...
              >
                ...
                <AnimatePresence>
                  {showUndoRedo && (
                    <>
                      ...
                      >
                        <FlipBackward className={iconDefault} />
                      </motion.button>
                      <motion.button
                        onClick={handleRedo}
                        disabled={!canRedo}
                      ...
                      >
                        <FlipForward className={iconDefault} />
                      </motion.button>
                    </>
                  )}
                </AnimatePresence>
                ...
                {onViewHistory && (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                  >
                    <ClockFastForward className={iconDefault} />
                  </motion.button>
                )}
                ...
                <motion.button
                  ...
                  className={`${iconButtonBase} bg-bg-brand-solid hover:bg-bg-brand-solid-hover disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-bg-brand-solid`}
                  title="Save (⌘S)"
                ...
                >
                  {isSaving ? (
                    <motion.span
                      className="block w-4 h-4 text-fg-white"
                      animate={{ rotate: 360 }}
                    ...
                    <Save01 className="w-4 h-4 text-white" />
                  )}
                </motion.button>
              </motion.div>
            ) : (
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            className="px-4 py-2 bg-bg-error-subtle border-b border-border-error flex items-center gap-2 text-sm text-fg-error-primary"
          >
        ...
        >
          {isEditing && viewMode === 'source' ? (
            ...
            <textarea
              ...
              className="w-full h-full min-h-[400px] p-4 font-mono text-sm bg-transparent text-fg-primary resize-none focus:outline-hidden"
      ...
      <AnimatePresence>
        {isEditing && (
          <motion.div 
            ...
            exit={{ height: 0, opacity: 0 }}
            transition={{ duration: 0.15 }}
            className="px-4 py-2 bg-bg-tertiary border-t border-border-primary flex items-center justify-between text-xs text-fg-tertiary"
          >
            <span>
              {hasUnsavedChanges ? 'Unsaved changes' : 'No changes'}
            </span>
            <div className="flex items-center gap-4">
              <span><kbd className="px-1.5 py-0.5 rounded bg-bg-tertiary text-fg-secondary">⌘Z</kbd> Undo</span>
              <span><kbd className="px-1.5 py-0.5 rounded bg-bg-tertiary text-fg-secondary">⌘S</kbd> Save</span>
              <span><kbd className="px-1.5 py-0.5 rounded bg-bg-tertiary text-fg-secondary">Esc</kbd> Cancel</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
function MarkdownPreview({ content }: { content: string }) {
  ...
  lines.forEach((line, index) => {
    ...
    if (line.startsWith('```')) {
      if (inCodeBlock) {
        elements.push(
          <pre key={`code-${index}`} className="bg-bg-tertiary rounded-lg p-4 overflow-x-auto my-4">
    ...
    // Blockquote
      ...
      elements.push(
        <blockquote key={index} className="border-l-4 border-border-brand pl-4 my-2 italic text-fg-secondary">
          {line.slice(1).trim()}
        </blockquote>
      );
    }
  ...
  return <div {...devProps('MarkdownPreview')} className="text-fg-primary">{elements}</div>;
}
