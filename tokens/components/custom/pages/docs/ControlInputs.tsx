...
function TextInput({ control, value, onChange }: ControlInputProps) {
  return (
    <input {...devProps('TextInput')}
      type="text"
      value={value ?? ''}
      onChange={(e) => onChange(e.target.value)}
      placeholder={control.defaultValue?.toString() || ''}
      className="w-full px-3 py-2 text-sm bg-os-bg-dark border border-os-border-dark rounded-lg text-brand-vanilla placeholder:text-os-text-secondary-dark focus:outline-hidden focus:border-brand-aperol/50 transition-colors"
    />
  );
}
...
function BooleanInput({ control, value, onChange }: ControlInputProps) {
  ...
  return (
    <div {...devProps('BooleanInput')} className="flex gap-1">
      <button
...
function ColorInput({ control, value, onChange }: ControlInputProps) {
  ...
  return (
    <div {...devProps('ColorInput')} className="flex items-center gap-2">
      ...
      <input
        ...
        placeholder="#000000"
        className="flex-1 px-3 py-2 text-sm bg-os-bg-dark border border-os-border-dark rounded-lg text-brand-vanilla placeholder:text-os-text-secondary-dark focus:outline-hidden focus:border-brand-aperol/50 transition-colors font-mono"
      />
    </div>
  );
}
...
function RangeInput({ control, value, onChange }: ControlInputProps) {
  ...
  return (
    <div {...devProps('RangeInput')} className="flex items-center gap-3">
...
function SelectInput({ control, value, onChange }: ControlInputProps) {
  ...
  return (
    <div {...devProps('SelectInput')} className="flex flex-col gap-1.5">
      {options.map((option) => (
        <label
...
export function ControlInput({ control, value, onChange }: ControlInputProps) {
  ...
  return (
    <div {...devProps('ControlInput')} className="space-y-2">
