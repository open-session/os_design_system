...
import { Check, Copy01, Download01, RefreshCcw01, ZoomIn, ZoomOut, Grid01 as Grid3X3, Maximize02 as Maximize2 } from '@untitledui-pro/icons/line';
...
`;
  ...
  if (!component) {
    return (
      <div className="flex items-center justify-center h-64 bg-bg-secondary rounded-xl border border-border-primary">
        <p className="text-fg-secondary">Select a component to preview</p>
      </div>
    );
  }
  ...
  const activeButtonClass = cn(buttonClass, 'bg-bg-tertiary text-fg-brand-primary');
  ...
  return (
    <div {...devProps('ComponentPreview')} className="space-y-6">
      {/* Preview Container */}
      <div className="bg-bg-secondary rounded-xl border border-border-primary overflow-hidden">
        {/* Toolbar */}
        <div className="flex items-center gap-1 px-4 py-2 border-b border-border-primary bg-bg-primary">
          ...
          <span className="text-xs text-fg-secondary min-w-[40px] text-center">
            {Math.round(zoom * 100)}%
          </span>
          <div className="w-px h-5 bg-border-primary mx-1" />
          <button onClick={handleCopyCode} className={codeCopied ? activeButtonClass : buttonClass} title="Copy code">
            {codeCopied ? <Check className="w-4 h-4 text-fg-success-primary" /> : <Copy01 className="w-4 h-4" />}
          </button>
        ...
        {allVariants.length > 1 && (
          <div className="flex items-center gap-2 px-4 py-3 border-t border-border-primary bg-bg-primary">
            <span className="text-xs text-fg-secondary uppercase tracking-wider font-medium">Variant:</span>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selectedVariant === variant.id
                      ? 'bg-bg-brand-solid text-fg-white border-border-brand-solid'
                      : 'bg-bg-secondary border-border-primary text-fg-secondary hover:text-fg-primary hover:border-border-secondary'
                  )}
                >
                  {variant.name}
                </button>
          ...
          {activeTab === 'props' ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border-primary">
                    <th className="text-left py-2 pr-4 text-fg-secondary font-medium">Name</th>
                    <th className="text-left py-2 pr-4 text-fg-secondary font-medium">Description</th>
                    <th className="text-left py-2 pr-4 text-fg-secondary font-medium">Default</th>
                    <th className="text-left py-2 text-fg-secondary font-medium">Type</th>
                  </tr>
                </thead>
                <tbody>
                  {component.controls.map(control => (
                    <tr key={control.name} className="border-b border-border-secondary">
                      <td className="py-3 pr-4">
                        <code className="text-fg-brand-primary">
                          {control.name}
                          {control.required && <span className="text-fg-error-primary">*</span>}
                        </code>
                      </td>
              ...
              {component.controls.length === 0 && (
                <p className="text-fg-secondary text-center py-4">
