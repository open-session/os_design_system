...
import { useChat, type QuickActionMetadata } from '@/hooks/useChat';
...
import { PlusesTexture } from '@/components/custom/shared/effects/PlusesTexture';
import { WelcomeHeader } from '@/components/custom/pages/home/WelcomeHeader';
import { QuickAccessPanels } from '@/components/custom/pages/home/QuickAccessPanels';
...
import { FollowUpInput, type FollowUpAttachment } from './FollowUpInput';
import { type SourceInfo, extractResourceCards } from './AnswerView';
import { type ImageResult } from './ImagesView';
...
import { CreatePostCopyForm } from './CreatePostCopyForm';
import { QuickActionFieldEditor, type FieldType, type EditorMode } from './QuickActionFieldEditor';
...
interface ParsedMessage {
  ...
  /** Quick action metadata if this message was part of a quick action */
...
export function ChatInterface() {
  ...
  const { messages, sendMessage, status, error, setMessages, regenerateLastAssistantMessage } = useChat({
    api: '/api/chat',
    onError: (err) => {
  ...
  // ID of the last assistant message — used for regenerate affordance
      ...
      requestAnimationFrame(() => {
        setTimeout(async () => {
          try {
            ...
            await sendMessage({ text: decodedQuery }, { body: { model: selectedModel, context, connectors: currentConnectors, writingStyle: currentWritingStyle?.id || null } });
          } catch (err) {
  ...
  }, [searchParams, router, sendMessage, setMessages, hasProcessedUrlParams, selectedModel, setSubmitError, webSearchEnabled, brandSearchEnabled, currentWritingStyle]);
  ...
  }, [quickActionConfig, cancelQuickAction, selectedModel, connectorSettings, currentWritingStyle, sendMessage, resetScrollTracking]);
        ...
        {hasMessages && (
          <div className="flex flex-col h-full">
            ...
            <div ref={scrollContainerRef} className="flex-1 overflow-y-auto relative" style={{ scrollbarGutter: 'stable' }}>
              ...
              <div className="sticky top-0 left-0 right-0 h-8 bg-gradient-to-b from-bg-primary to-transparent pointer-events-none z-10" />
              <div className={`max-w-3xl mx-auto px-4 ${showQuickActionForm ? '' : '-mt-8'}`}>
                          ...
                          return (
                            <div key={message.id}>
                              <ChatContent
                                ...
                                onRegenerate={
                                  nextMessage.id === lastAssistantMsgId && status === 'ready'
                ...
                {(error || submitError) && !parsedMessages.some(m => m.role === 'assistant' && m.content.length > 50) && (
                  <div className="py-4">
                    <div className="bg-bg-error-primary border border-border-error rounded-xl px-4 py-3 text-fg-error-primary text-sm flex items-start gap-3">
            ...
            >
                        ...
                        <button
                          ...
                          className={`p-2 rounded-lg transition-all ${
                            (input.trim() || attachments.length > 0) && !isLoading
                              ? 'bg-bg-brand-solid text-white hover:bg-bg-brand-solid-hover'
                              : 'text-fg-tertiary-subtle cursor-not-allowed'
                          }`}
              ...
              <AnimatePresence>
                {(error || submitError) && (
                  ...
                  >
                    <div className="bg-bg-error-primary border border-border-error rounded-xl px-4 py-3 text-fg-error-primary text-sm flex items-start gap-3 text-left">
                      <AlertCircle className="w-5 h-5 flex-shrink-0 mt-0.5" />
                      <div>
                        <p className="font-medium">Error</p>
                        <p className="mt-1">{error?.message || submitError}</p>
