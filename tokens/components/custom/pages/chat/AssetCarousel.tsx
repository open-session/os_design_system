...
import { ChevronLeft, ChevronRight, Download01 as Download, SearchLg, ChevronDown, LinkExternal01 } from '@untitledui-pro/icons/line';
...
function Dropdown({ 
  ...
  return (
    <div {...devProps('Dropdown')} ref={dropdownRef} className="relative">
      <button
        onClick={(e) => {
          e.stopPropagation();
          setIsOpen(!isOpen);
        }}
        className="flex items-center gap-1 px-1.5 py-1 rounded-md bg-bg-secondary border border-border-secondary hover:bg-bg-tertiary hover:border-border-primary transition-colors text-[10px]"
        aria-label="Select option"
      >
        {Icon && <Icon className="w-3 h-3 text-fg-tertiary" />}
        <ChevronDown className={`w-2 h-2 text-fg-tertiary transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      
      {isOpen && (
        <div className="absolute top-full right-0 mt-1 w-20 rounded-lg bg-bg-secondary border border-border-secondary shadow-lg z-30 overflow-hidden">
          {options.map((option) => (
            <button
              key={option.value}
              onClick={(e) => {
...
function LogoCard({ logo }: { logo: LogoAsset }) {
  ...
  const bgClass = colorVariant === 'vanilla' 
    ...
    ? 'bg-bg-primary'
    : 'bg-gradient-to-br bg-bg-primary via-[#2a2a2a] bg-bg-primary';
  ...
  return (
    <div {...devProps('FontCard')} className="group flex-shrink-0 w-[180px] sm:w-[200px] rounded-lg overflow-hidden border border-border-secondary bg-bg-secondary transition-all duration-200 hover:border-border-brand">
      ...
      <div className="bg-bg-tertiary px-2 py-1.5 border-b border-border-secondary flex items-center justify-between">
        ...
        >
          <Download className="w-3 h-3 text-fg-tertiary" />
        </button>
      </div>
      
      {/* Specimen */}
      <div className="bg-bg-primary p-4 min-h-[100px] flex items-center justify-center">
        <p className={`${fontClass} text-[18px] sm:text-[20px] text-fg-primary text-center leading-tight`}>
          {font.specimen}
        </p>
      </div>
      
      {/* Weights */}
      <div className="px-2 py-1.5 border-t border-border-secondary">
...
export function AssetCarousel({ type, className = '' }: AssetCarouselProps) {
  ...
  return (
    <div {...devProps('AssetCarousel')} className={`relative ${className}`}>
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        <Link
          href={pagePath}
          className="flex items-center gap-1 text-[11px] text-fg-brand-primary hover:text-fg-brand-primary-hover transition-colors"
        >
          <span>View all</span>
      ...
      <div className="relative group">
        ...
        {showLeftArrow && (
          <button
            onClick={() => scroll('left')}
            className="hidden sm:flex absolute left-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 items-center justify-center rounded-full bg-bg-secondary border border-border-secondary shadow-lg hover:bg-bg-tertiary transition-all opacity-0 group-hover:opacity-100"
        ...
        {showRightArrow && (
          <button
            ...
            className="hidden sm:flex absolute right-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 items-center justify-center rounded-full bg-bg-secondary border border-border-secondary shadow-lg hover:bg-bg-tertiary transition-all opacity-0 group-hover:opacity-100"
