...
interface UserMessageBubbleProps {
  ...
  /** Whether this is the last user message in the thread */
...
export function UserMessageBubble({
  ...
  // Handle copy with feedback - must be before early return for rules-of-hooks
  ...
  return (
    <>
      <div {...devProps('UserMessageBubble')} className="flex justify-end mb-6">
        <div className="bg-bg-secondary rounded-2xl max-w-[80%] relative overflow-hidden">
            ...
            <div className="p-2 pb-0">
              <div className={`grid gap-2 ${
                ...
                {attachments.map((attachment) => (
                  <div
                    key={attachment.id}
                    className="relative group aspect-video rounded-lg overflow-hidden bg-bg-primary"
                  >
                    <Image
                      src={attachment.data}
          ...
          {query && (
            ...
            >
              <p className="text-[15px] text-fg-primary whitespace-pre-wrap">
                {query}
              </p>
            </div>
          )}
          ...
          {hasOverflow && !isExpanded && (
            <div className="absolute bottom-0 left-0 right-0">
              {/* Gradient fade */}
              <div className="h-12 bg-gradient-to-t from-bg-secondary via-bg-secondary to-transparent" />
              
              {/* Show more button */}
              <button
                onClick={() => setIsExpanded(true)}
                className="w-full flex items-center justify-center gap-1 py-2 bg-bg-secondary text-xs text-fg-tertiary hover:text-fg-secondary transition-colors"
              >
                <span>Show more</span>
          ...
          {hasOverflow && isExpanded && (
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-1 py-2 border-t border-border-secondary text-xs text-fg-tertiary hover:text-fg-secondary transition-colors"
            >
              <span>Show less</span>
