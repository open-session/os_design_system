...
function renderInlineMarkdown(text: string): React.ReactNode {
  ...
  while ((match = boldRegex.exec(text)) !== null) {
    ...
    parts.push(
      <strong key={`bold-${match.index}`} className="font-bold text-fg-primary">
        {match[1]}
      </strong>
    );
...
interface AnswerViewProps {
  ...
  sources: SourceInfo[];
  isStreaming?: boolean;
  /** Whether the typewriter is still revealing buffered content */
  isRevealing?: boolean;
  ...
  return (
    <div {...devProps('AnswerView')}>
      ...
      <div className="space-y-3">
          ...
          if (section.type === 'heading') {
            ...
            if (level === 1) {
              const h1Styles = hasHeadingHierarchy
                ...
                : "font-display font-bold text-chat-h1 text-fg-primary pl-3 border-l-2 border-border-brand mt-6 mb-3";
              return (
            ...
            if (level === 2) {
              return (
                <h2 key={idx} className="font-display font-semibold text-chat-h2 text-fg-primary pl-3 border-l-2 border-border-brand mt-5 mb-2.5">
                  {renderInlineMarkdown(section.content)}
                </h2>
              );
            }
        ...
        {/* Inline blinking cursor — visible while typewriter is revealing content */}
        {(isRevealing || isStreaming) && sections.length > 0 && (
          <motion.span
            className="inline-block w-[2px] h-[1em] bg-fg-primary ml-0.5 align-text-bottom rounded-full"
...
export function parseContentToSections(
  ...
  // Strip any raw <thinking>...</thinking> tags the model may emit as literal output.
  // Prompt-injection hygiene — the API never returns thinking blocks anymore.
