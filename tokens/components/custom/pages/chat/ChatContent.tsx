...
import { CanvasPreviewBubble } from '@/components/custom/shared/overlays/CanvasPreviewBubble';
...
export function ChatContent({
  ...
  // Smooth typewriter effect for streaming responses
  // Buffers incoming text and reveals it at a constant rate via rAF
  const { displayedContent, isRevealing } = useStreamingTypewriter(
  ...
  const sections = React.useMemo(() => {
    ...
    // Use typewriter-revealed content for smooth display during streaming
    return parseContentToSections(displayedContent, sources);
  }, [displayedContent, preamble, sources, canvasResponse]);
  ...
  return (
    <div {...devProps('ChatContent')} className="py-6">
      <AnswerView
        query={query}
        sections={sections}
        sources={sources}
