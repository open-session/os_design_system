...
function FormatCheckbox({ format, label, checked, onChange }: FormatCheckboxProps) {
  return (
    <label {...devProps('FormatCheckbox')} className="flex items-center gap-2 cursor-pointer">
      <input
        type="checkbox"
        checked={checked}
        onChange={(e) => onChange(e.target.checked)}
        className="w-4 h-4 rounded border-border-primary text-fg-brand-primary focus:ring-border-brand-solid focus:ring-offset-0"
      />
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div {...devProps('QuickActionFieldEditor')} className="bg-bg-primary rounded-xl shadow-xl ring-1 ring-border-secondary">
              {/* Header */}
              <div className="flex items-center justify-between px-5 py-4 border-b border-border-secondary">
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="p-1.5 text-fg-tertiary hover:text-fg-primary hover:bg-bg-tertiary rounded-md transition-colors"
                >
              ...
              <div className="px-5 py-4 space-y-4">
                {mode === 'delete' ? (
                  ...
                  <div className="space-y-4">
                    <div className="flex items-start gap-3 p-4 bg-bg-error-primary rounded-lg">
                      <AlertTriangle className="w-5 h-5 text-fg-error-primary flex-shrink-0 mt-0.5" />
                      <div className="space-y-1">
                        <p className="text-sm font-medium text-fg-error-primary">
                          Delete &quot;{item?.label}&quot;?
                        </p>
                  ...
                  <>
                    ...
                    <div className="space-y-1.5">
                      <label className="text-sm font-medium text-fg-secondary">
                        Label <span className="text-fg-error-primary">*</span>
                      </label>
                      <input
                        type="text"
                        value={label}
                        ...
                        className="w-full px-3 py-2 bg-bg-primary rounded-lg text-sm text-fg-primary placeholder:text-fg-quaternary ring-1 ring-border-primary focus:outline-hidden focus:ring-1 focus:ring-brand focus:shadow-focus-ring"
                        autoFocus
                      />
                    </div>
                    ...
                    {fieldType === 'channel' && (
                      <>
                        <div className="space-y-1.5">
                          <label className="text-sm font-medium text-fg-secondary">
                            Short Label <span className="text-fg-error-primary">*</span>
                          </label>
                          <input
                            type="text"
                            value={shortLabel}
                            ...
                            placeholder="e.g., IG"
                            maxLength={5}
                            className="w-full px-3 py-2 bg-bg-primary rounded-lg text-sm text-fg-primary placeholder:text-fg-quaternary ring-1 ring-border-primary focus:outline-hidden focus:ring-1 focus:ring-brand focus:shadow-focus-ring uppercase"
                          />
                        ...
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-fg-secondary">
                            Supported Formats <span className="text-fg-error-primary">*</span>
                          </label>
                          <div className="space-y-2">
