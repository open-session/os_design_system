...
export function RelatedQuestions({
  ...
  if (isLoading) {
    return (
      <div {...devProps('RelatedQuestions')} className="mt-6">
      ...
      <div className="space-y-1">
        {questions.map((question, idx) => (
          <button
            ...
            className="w-full flex items-start gap-3 py-2.5 px-3 text-left hover:bg-bg-secondary transition-colors group rounded-lg border border-transparent hover:border-border-primary/30"
          >
            <CornerDownRight className="w-4 h-4 text-fg-tertiary group-hover:text-fg-brand-primary mt-0.5 flex-shrink-0 transition-colors" />
            <span className="text-[14px] text-fg-primary/80 group-hover:text-fg-primary transition-colors leading-relaxed">
              {question}
            </span>
          </button>
        ))}
