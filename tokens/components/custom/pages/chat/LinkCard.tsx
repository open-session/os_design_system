...
import { DotsHorizontal, Globe01, File01, Hexagon01, Rss01, Type01, BookOpen01, Camera01, Fingerprint01, Image01, PenTool01, MessageCircle01, MessageSquare01, LayersTwo01, XClose, LinkExternal01, FolderPlus, Check, Copy01, ArrowRight, SearchLg } from '@untitledui-pro/icons/line';
...
export function LinkCard({
  ...
  const BrandIcon = useMemo(() => {
    if (link.type === 'brand' && link.iconName) {
      return ICON_MAP[link.iconName] || Hexagon01;
    }
    return null;
  }, [link.type, link.iconName]);

  // Type01-specific fallback icon
  ...
  // Type01-specific color
  const typeColor = useMemo(() => {
    switch (link.type) {
      case 'brand':
        return 'text-fg-brand-primary';
      case 'news':
      ...
      >
        ...
        >
          ...
          <div className="flex items-start justify-between mb-2">
            {/* Favicon/Icon */}
            <div className="w-8 h-8 rounded-lg bg-bg-tertiary flex items-center justify-center flex-shrink-0 overflow-hidden">
          ...
          <h3 className="text-[13px] font-medium text-fg-primary line-clamp-2 leading-snug flex-grow">
            {displayTitle}
          </h3>

          {/* Domain + citation number - always at bottom */}
          <div className="flex items-center gap-1.5 mt-2">
            <span className="text-[11px] text-fg-quaternary truncate">
              {displayDomain}
            </span>
                ...
                <>
                  ...
                  >
                    ...
                    <div className="px-2 py-1">
                      <p className="text-[10px] font-medium text-fg-quaternary uppercase tracking-wider px-2 mb-1">
                        Add to Space
                      </p>
                      {spaces.length === 0 ? (
                        <p className="text-xs text-fg-tertiary px-2 py-1">No spaces yet</p>
                      ) : (
                          ...
                          {spaces.slice(0, 5).map((space) => (
                            <button
                              ...
                              className="
                                ...
                                hover:bg-bg-tertiary rounded-md
                                transition-colors
                              "
                            >
    ...
    >
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-[13px] text-fg-secondary group-hover:text-fg-primary transition-colors truncate">
            {displayTitle}
          </h3>
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-[10px] text-fg-quaternary truncate">
              {displayDomain}
            </span>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          >
            <button
              ...
              className="p-1 rounded hover:bg-bg-secondary text-fg-tertiary hover:text-fg-primary transition-colors"
            >
              <DotsHorizontal className="w-4 h-4" />
            </button>
          </div>
...
export function LinkCardSkeleton({ variant = 'grid' }: { variant?: 'grid' | 'list' }) {
  if (variant === 'grid') {
    return (
      <div {...devProps('LinkCardSkeleton')} className="h-full min-h-[120px] p-3 rounded-xl bg-bg-secondary border border-border-secondary animate-pulse flex flex-col">
        <div className="flex items-start justify-between mb-2">
          <div className="w-8 h-8 rounded-lg bg-bg-tertiary" />
          <div className="w-10 h-4 rounded bg-bg-tertiary" />
        </div>
        <div className="h-4 w-3/4 rounded bg-bg-tertiary mb-1 flex-grow" />
        <div className="h-3 w-1/2 rounded bg-bg-tertiary mt-auto" />
      </div>
    );
  }

  return (
    <div {...devProps('LinkCardSkeleton')} className="flex items-center gap-3 px-3 py-2.5 animate-pulse">
      <div className="w-5 h-5 rounded bg-bg-tertiary" />
      <div className="flex-1">
        <div className="h-4 w-3/4 rounded bg-bg-tertiary mb-1" />
