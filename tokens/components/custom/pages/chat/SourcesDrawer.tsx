    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          <motion.div
            {...devProps('SourcesDrawer')}
            variants={drawerVariants}
            initial="hidden"
            animate="visible"
            exit="exit"
            className="fixed right-0 top-14 bottom-0 w-[400px] max-w-[90vw] bg-bg-primary z-[101] flex flex-col shadow-2xl"
          >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-border-secondary">
          <h2 className="text-[15px] font-semibold text-fg-primary">
            Sources
            <span className="text-xs text-fg-tertiary/50 font-normal ml-2">
              {totalCount}
            </span>
          </h2>
          <button
            onClick={onClose}
            className="p-2 rounded-lg text-fg-tertiary hover:text-fg-primary hover:bg-bg-secondary transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <div className="pt-4 pb-2">
              <div className="px-5 mb-3">
                <div className="flex items-center gap-2">
                  <Hexagon01 className="w-3.5 h-3.5 text-fg-tertiary/50" />
                  <span className="text-[11px] font-medium text-fg-tertiary/60 uppercase tracking-wider">
                    Brand Context
                  </span>
                </div>
              </div>
                ...
                {resourceCards.map((card, idx) => (
                  ...
                  >
                    ...
                    <div className="w-8 h-8 rounded-lg bg-bg-secondary flex items-center justify-center flex-shrink-0">
                      <Hexagon01 className="w-4 h-4 text-fg-tertiary/60" />
                    </div>

                    {/* Content */}
                    <div className="flex-1 min-w-0 pt-0.5">
                      <h3 className="text-[13px] font-medium text-fg-primary group-hover:text-fg-primary transition-colors leading-snug">
                        {card.title}
                      </h3>
                      <p className="text-[12px] text-fg-tertiary/50 mt-0.5 line-clamp-1">
                        {card.description}
                      </p>
                    </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="w-8 h-8 rounded-lg bg-cyan-500/10 flex items-center justify-center flex-shrink-0">
                        <Rss01 className="w-4 h-4 text-cyan-400" />
                      </div>

                      {/* Content */}
                      <div className="flex-1 min-w-0 pt-0.5">
                        {/* Title */}
                        <h3 className="text-[13px] font-medium text-fg-primary group-hover:text-cyan-400 transition-colors line-clamp-2 leading-snug">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          {categoryConfig && (
                            <span className={`text-[9px] px-1.5 py-0.5 rounded-full bg-bg-secondary ${categoryConfig.color}`}>
                        ...
                        {/* Published time */}
                        {source.publishedAt && (
                          <p className="text-[10px] text-fg-tertiary/40 mt-1">
                            {source.publishedAt}
                          </p>
                        )}
                      </div>
          ...
          {hasWebSources && (
            <div className="pt-4 pb-2">
              <div className="px-5 mb-3">
                <div className="flex items-center gap-2">
                  <Globe01 className="w-3.5 h-3.5 text-fg-tertiary/60" />
                  <span className="text-[11px] font-medium text-fg-tertiary/60 uppercase tracking-wider">
                    Citations
                  </span>
                  <span className="text-[10px] text-fg-tertiary/40 ml-auto">
                    {webSources.length}
                  </span>
                </div>
              </div>
                ...
                {webSources.map((source, idx) => (
                  ...
                  >
                    ...
                    <div className="w-8 h-8 rounded-lg bg-bg-secondary flex items-center justify-center flex-shrink-0 overflow-hidden">
                      ...
                      <Globe01 className="w-4 h-4 text-fg-tertiary/50 hidden" />
                    </div>

                    {/* Content */}
                    <div className="flex-1 min-w-0 pt-0.5">
                      {/* Title - use extracted title, fallback to site name */}
                      <h3 className="text-[13px] font-medium text-fg-primary group-hover:text-fg-brand-primary transition-colors line-clamp-2 leading-snug">
                        {source.title && source.title !== source.name ? source.title : `Article from ${source.name}`}
                      </h3>

                      {/* Snippet - show if available */}
                      {source.snippet && (
                        <p className="text-[12px] text-fg-secondary/70 mt-1 line-clamp-2 leading-relaxed">
                          {source.snippet}
                        </p>
                      )}
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className="text-[11px] text-fg-tertiary/50 truncate">
                          {source.name}
                        </span>
                        <span className="text-[10px] text-fg-tertiary/30 bg-bg-secondary px-1.5 py-0.5 rounded">
                          [{idx + 1}]
                        </span>
                      </div>
                    </div>
