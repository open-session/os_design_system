...
import { Image01 as ImageIcon, XClose, LinkExternal01, Globe01 } from '@untitledui-pro/icons/line';
...
export function ImagesView({ query, images }: ImagesViewProps) {
  ...
  return (
    <div {...devProps('ImagesView')}>
      <div className="space-y-4">
        {/* Search query header */}
        <p className="text-sm text-fg-tertiary">
          Image results for: <span className="font-medium text-fg-primary">{query}</span>
        </p>
...
function ImageCard({
  ...
  if (error) {
    return (
      <div {...devProps('ImageCard')} className="aspect-square rounded-lg bg-bg-secondary flex items-center justify-center">
        <ImageIcon className="w-8 h-8 text-fg-quaternary opacity-40" />
      </div>
    );
  }
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <div className="flex items-center gap-1.5">
          <Globe01 className="w-3 h-3 text-white/70" />
...
function ImageLightbox({
  ...
  return (
    ...
    >
      ...
      >
        <XClose className="w-5 h-5 text-white" />
      </button>
