...
import { Copy01, Check, Key01, ChevronDown, ChevronUp, AlertCircle, CheckCircle, RefreshCw01 as RefreshCw, LinkExternal01 as ExternalLink } from '@untitledui-pro/icons/line';
...
function NewKeyModal({
  ...
  return (
    <div {...devProps('NewKeyModal')} className="fixed inset-0 z-50 flex items-center justify-center">
      <div
        className="absolute inset-0 bg-black/50 backdrop-blur-sm"
        onClick={onClose}
      />
      
      <div className="relative bg-bg-primary rounded-xl border border-border-secondary shadow-2xl max-w-lg w-full mx-4 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-fg-tertiary">{keyName}</p>
          </div>
        </div>

        <div className="bg-bg-secondary-alt border border-border-secondary rounded-lg p-4 mb-4">
          <div className="flex items-center justify-between gap-3">
            <code className="flex-1 text-sm font-mono text-fg-primary break-all">
              {keyValue}
            </code>
            <button
              onClick={copyKey}
        ...
        <button
          ...
          className="
            w-full py-2 px-4
            bg-bg-tertiary
            text-fg-primary text-sm font-medium
            rounded-lg
...
function AdvancedSettingsDropdown({
  ...
  return (
    <div {...devProps('AdvancedSettingsDropdown')} className="bg-blue-500/10 border border-blue-500/20 rounded-lg overflow-hidden">
      <button
      ...
      {isOpen && (
        <div className="px-3 pb-3 space-y-2">
          ...
          <div className="flex items-center justify-between gap-2 p-2 bg-bg-primary rounded border border-border-secondary">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-fg-tertiary mb-0.5">Authorization Header</p>
              <code className="text-xs font-mono text-fg-primary break-all">{bearerToken}</code>
            </div>
            <button
              onClick={() => onCopy(bearerToken, 'bearerToken')}
              className="p-1.5 text-fg-quaternary hover:text-fg-tertiary hover:bg-bg-tertiary rounded transition-colors flex-shrink-0"
            >
              {copied === 'bearerToken' ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy01 className="w-3.5 h-3.5" />}
            </button>
          </div>
          
          <div className="flex items-center justify-between gap-2 p-2 bg-bg-primary rounded border border-border-secondary">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-fg-tertiary mb-0.5">API Key (raw)</p>
              <code className="text-xs font-mono text-fg-primary break-all">{apiKey || 'YOUR_API_KEY'}</code>
            </div>
            <button
              onClick={() => onCopy(apiKey || 'YOUR_API_KEY', 'apiKey')}
              className="p-1.5 text-fg-quaternary hover:text-fg-tertiary hover:bg-bg-tertiary rounded transition-colors flex-shrink-0"
            >
              {copied === 'apiKey' ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy01 className="w-3.5 h-3.5" />}
            </button>
          </div>
        </div>
      )}
...
function UrlInstructions({ 
  ...
  const instructions: Record<Platform, React.ReactNode> = {
    claude: (
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-2 p-2 bg-bg-primary rounded-lg border border-border-secondary">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-fg-tertiary mb-0.5">Server URL</p>
              <code className="text-xs font-mono text-fg-primary break-all">{serverUrl}</code>
            </div>
            <button
              onClick={() => onCopy(serverUrl, 'url')}
              className="p-1.5 text-fg-quaternary hover:text-fg-tertiary hover:bg-bg-tertiary rounded transition-colors flex-shrink-0"
            >
              {copied === 'url' ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy01 className="w-3.5 h-3.5" />}
            </button>
          </div>
        </div>
    ...
    cursor: (
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between gap-2 p-2 bg-bg-primary rounded-lg border border-border-secondary">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-fg-tertiary mb-0.5">API Key / Bearer Token</p>
              <code className="text-xs font-mono text-fg-primary break-all">{apiKey || 'YOUR_API_KEY'}</code>
            </div>
            <button
              ...
              className="p-1.5 text-fg-quaternary hover:text-fg-tertiary hover:bg-bg-tertiary rounded transition-colors flex-shrink-0"
            >
              {copied === 'token' ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy01 className="w-3.5 h-3.5" />}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between gap-2 p-2 bg-bg-primary rounded-lg border border-border-secondary">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-fg-tertiary mb-0.5">Authorization Header</p>
              <code className="text-xs font-mono text-fg-primary break-all">Bearer {apiKey || 'YOUR_API_KEY'}</code>
            </div>
            <button
              onClick={() => onCopy(`Bearer ${apiKey || 'YOUR_API_KEY'}`, 'auth')}
              className="p-1.5 text-fg-quaternary hover:text-fg-tertiary hover:bg-bg-tertiary rounded transition-colors flex-shrink-0"
            >
              {copied === 'auth' ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy01 className="w-3.5 h-3.5" />}
            </button>
          </div>
        </div>
      </div>
    ...
    gemini: (
      <div className="space-y-3">
        <ol className="text-sm text-fg-secondary space-y-2 list-decimal list-inside">
          <li>Open <strong>Google AI Studio</strong> at <a href="https://aistudio.google.com" target="_blank" rel="noopener noreferrer" className="text-fg-brand hover:underline">aistudio.google.com</a></li>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between gap-2 p-2 bg-bg-primary rounded-lg border border-border-secondary">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-fg-tertiary mb-0.5">API Key</p>
              <code className="text-xs font-mono text-fg-primary break-all">{apiKey || 'YOUR_API_KEY'}</code>
            </div>
            <button
              onClick={() => onCopy(apiKey || 'YOUR_API_KEY', 'token')}
    ...
    chatgpt: (
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-2 p-2 bg-bg-primary rounded-lg border border-border-secondary">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-fg-tertiary mb-0.5">API Endpoint</p>
              <code className="text-xs font-mono text-fg-primary break-all">{serverUrl}</code>
            </div>
            <button
              onClick={() => onCopy(serverUrl, 'url')}
          ...
          <div className="flex items-center justify-between gap-2 p-2 bg-bg-primary rounded-lg border border-border-secondary">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-fg-tertiary mb-0.5">Bearer Token</p>
              <code className="text-xs font-mono text-fg-primary break-all">{apiKey || 'YOUR_API_KEY'}</code>
            </div>
            <button
              onClick={() => onCopy(apiKey || 'YOUR_API_KEY', 'token')}
...
function ConfigInstructions({ 
  ...
  if (!info.config) {
    return (
      <div {...devProps('ConfigInstructions')} className="p-3 bg-bg-tertiary rounded-lg">
  ...
  return (
    <div {...devProps('ConfigInstructions')} className="space-y-3">
      <p className="text-sm text-fg-secondary">{info.instructions}</p>
      
      <div className="relative">
        <div className="absolute top-2 right-2">
          <button
            onClick={() => onCopy(info.config, 'config')}
            className="p-1.5 text-fg-quaternary hover:text-fg-tertiary hover:bg-bg-quaternary rounded transition-colors"
            title="Copy config"
          >
            {copied === 'config' ? <Check className="w-3.5 h-3.5 text-green-500" /> : <Copy01 className="w-3.5 h-3.5" />}
          </button>
        </div>
        <pre className="text-xs font-mono text-fg-tertiary bg-bg-primary border border-border-secondary p-3 pr-12 rounded-lg overflow-x-auto">
          {info.config}
        </pre>
      </div>
      
      <p className="text-xs text-fg-quaternary">
        File location: <code className="bg-bg-tertiary px-1 rounded">{info.path}</code>
      </p>
    </div>
  );
}
...
function QuickSetupCard({ 
  ...
  return (
    <div {...devProps('QuickSetupCard')} className="bg-bg-secondary-alt border border-border-secondary rounded-xl overflow-hidden">
      {/* Header */}
      <div 
      ...
      >
        <div>
          <h4 className="text-sm font-semibold text-fg-primary">Quick Setup</h4>
          <p className="text-xs text-fg-tertiary">Connect your AI tools in minutes</p>
        </div>
        {showConfig ? (
      ...
      {showConfig && (
        <div className="px-4 pb-4 space-y-5 border-t border-border-secondary pt-4">
          {/* Step 1: API Key */}
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="flex items-center justify-center w-5 h-5 bg-bg-brand-solid text-white text-xs font-bold rounded-full">1</span>
              <span className="text-sm font-medium text-fg-secondary">Get your API key</span>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <span className="flex items-center justify-center w-5 h-5 bg-bg-brand-solid text-white text-xs font-bold rounded-full">2</span>
              <span className="text-sm font-medium text-fg-secondary">Connect your AI tool</span>
            </div>
            
            <div className="ml-7 space-y-3" onClick={(e) => e.stopPropagation()}>
              {/* Platform selector tabs - outside container */}
              <div className="relative inline-flex items-center gap-0.5 p-1 rounded-lg bg-bg-tertiary">
              ...
              <div className="bg-bg-tertiary rounded-lg p-4 space-y-4">
                {currentPlatform.supportsUrl && (
                  <div className="space-y-2">
                    <h5 className="text-xs font-semibold text-fg-primary uppercase tracking-wide">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <span className="flex items-center justify-center w-5 h-5 bg-bg-brand-solid text-white text-xs font-bold rounded-full">3</span>
              <span className="text-sm font-medium text-fg-secondary">Verify in your AI tool</span>
            </div>
            <div className="ml-7">
...
export function McpServerSettings({ brandId }: McpServerSettingsProps) {
  ...
  if (isLoading) {
    return (
      <div {...devProps('McpServerSettings')} className="flex items-center justify-center py-12">
  ...
  if (error) {
    return (
      <div {...devProps('McpServerSettings')} className="bg-red-500/10 border border-red-500/20 rounded-lg p-4">
  ...
  return (
    <div {...devProps('McpServerSettings')} className="space-y-6">
      {/* Header Section */}
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 pb-5 border-b border-border-secondary">
        <div>
          <h3 className="text-lg font-semibold text-fg-primary">
            BOS MCP Server
          </h3>
        ...
        <div className="flex items-center gap-3">
          {isTogglingServer && (
            <RefreshCw className="w-4 h-4 animate-spin text-fg-tertiary" />
          )}
          <Toggle
      ...
      {!config?.isEnabled && (
        <div className="flex items-center gap-2 p-3 bg-bg-tertiary border border-border-secondary rounded-lg">
          <AlertCircle className="w-4 h-4 text-fg-tertiary" />
