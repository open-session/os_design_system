...
function FormField({
  ...
  return (
    <div {...devProps('FormField')} className="space-y-1.5">
      <label className="block text-sm font-medium text-fg-secondary">
        {label}
        {required && <span className="text-fg-error-primary ml-0.5">*</span>}
      </label>
      {children}
      {hint && (
        <p className="text-xs text-fg-quaternary">{hint}</p>
      )}
    </div>
  );
}
...
function ToolsList({ tools }: { tools: McpTool[] }) {
  if (tools.length === 0) {
    return (
      <p {...devProps('ToolsList')} className="text-sm text-fg-tertiary">
  ...
  return (
    <div {...devProps('ToolsList')} className="space-y-2 max-h-48 overflow-y-auto">
      {tools.map((tool) => (
        <div
          key={tool.name}
          className="flex items-start gap-2 p-2 bg-bg-tertiary rounded-lg"
        >
          <Zap className="w-4 h-4 text-fg-brand-primary flex-shrink-0 mt-0.5" />
...
export function McpConnectionModal({
  ...
  return (
    <div {...devProps('McpConnectionModal')} className="fixed inset-0 z-50 flex items-center justify-center">
      {/* Backdrop */}
      <div
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg mx-4 bg-bg-primary rounded-xl shadow-2xl border border-border-secondary max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-border-secondary">
          ...
          <button
            onClick={onClose}
            className="p-2 text-fg-quaternary hover:text-fg-tertiary hover:bg-bg-tertiary rounded-lg transition-colors"
          >
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <FormField label="Server URL" required hint="The MCP server endpoint URL">
            <div className="flex items-center gap-2">
              <Globe01 className="w-4 h-4 text-fg-quaternary" />
              <input
                type="url"
                value={serverUrl}
          ...
          {authType === 'bearer' && (
            <FormField label="Bearer Token" required>
              <div className="flex items-center gap-2">
                <Key01 className="w-4 h-4 text-fg-quaternary" />
                <input
                  type="password"
                  value={token}
              ...
              {testResult && (
                <div
                  className={`
                    ...
                    ${
                      testResult.success
                        ? 'bg-bg-success-primary border-border-success'
                        : 'bg-bg-error-primary border-border-error'
                    }
                  `}
                >
                  <div className="flex items-center gap-2 mb-2">
                    {testResult.success ? (
                      <CheckCircle className="w-4 h-4 text-fg-success-primary" />
                    ) : (
                      <XCircle className="w-4 h-4 text-fg-error-primary" />
                    )}
                    <span
                      className={`text-sm font-medium ${
                        testResult.success ? 'text-fg-success-primary' : 'text-fg-error-primary'
                      }`}
                    >
                  ...
                  {testResult.error && (
                    <p className="text-sm text-fg-error-primary">{testResult.error}</p>
                  )}
                </div>
              )}
            </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border-secondary">
          ...
          <button
            ...
            className="
              flex items-center gap-2 px-4 py-2
              bg-bg-brand-solid
              text-fg-white text-sm font-medium
              rounded-lg
            ...
            "
          >
            {isSaving && <Loading01 className="w-4 h-4 animate-spin" />}
