...
function WeightSelect({ value, onChange, disabled }: WeightSelectProps) {
  ...
  return (
    <div {...devProps('WeightSelect')} ref={containerRef} className="relative">
      <button
        ref={buttonRef}
        onClick={() => !disabled && setIsOpen(!isOpen)}
        disabled={disabled}
        className={`flex items-center gap-1.5 px-2 py-1 text-xs rounded-md border border-border-primary bg-bg-secondary transition-colors min-w-[90px] ${
          disabled ? 'opacity-50 cursor-not-allowed' : 'hover:bg-bg-tertiary'
        }`}
      >
        <span className="text-fg-primary truncate">{currentWeight.value}</span>
      ...
      <AnimatePresence>
        {isOpen && !disabled && (
          <motion.div
            ...
            className={`absolute left-0 z-[100] py-1 min-w-[140px] max-h-[260px] overflow-auto rounded-lg border border-border-primary bg-bg-primary shadow-xl ${
          ...
          >
            {WEIGHT_OPTIONS.map((option) => (
              ...
              >
                <span className="w-7 font-mono text-[10px] text-fg-tertiary">{option.value}</span>
...
function UsageSelect({ value, onChange, disabled }: UsageSelectProps) {
  ...
  const getUsageColor = (usage: string) => {
    switch (usage) {
      case 'display':
        return 'bg-bg-brand-solid/10 text-fg-brand-primary';
      case 'body':
  ...
  return (
    <div {...devProps('UsageSelect')} ref={containerRef} className="relative">
      <button
        ref={buttonRef}
        onClick={() => !disabled && setIsOpen(!isOpen)}
        disabled={disabled}
      ...
      <AnimatePresence>
        {isOpen && !disabled && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: dropdownPosition === 'bottom' ? -4 : 4 }}
            className={`absolute left-0 z-[100] py-1 min-w-[160px] rounded-lg border border-border-primary bg-bg-primary shadow-xl ${
              dropdownPosition === 'bottom' 
                ? 'top-full mt-1' 
                : 'bottom-full mb-1'
            }`}
          >
            {USAGE_OPTIONS.map((option) => (
              ...
              >
                ...
                <span className="text-[10px] text-fg-muted">{option.description}</span>
                {value === option.value && <Check className="w-3 h-3 ml-auto text-fg-brand-primary" />}
              </button>
            ))}
          </motion.div>
        )}
...
function StaticFontRow({ font, onDownload }: StaticFontRowProps) {
  ...
  return (
    <tr {...devProps('StaticFontRow')} className="group border-b border-border-secondary hover:bg-bg-secondary transition-colors">
      {/* Preview */}
      <td className="py-2 px-2 sm:px-3 w-[50px] sm:w-[60px]">
        <div 
          className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-bg-secondary border border-border-primary flex items-center justify-center overflow-hidden"
          style={{ fontWeight: parseInt(font.weight) }}
        >
          <span className="text-base sm:text-lg text-fg-primary">Aa</span>
        </div>
      </td>
      ...
      <td className="py-2 px-2 sm:px-3 w-[100px] sm:w-[140px] hidden sm:table-cell">
        <span className="text-[10px] sm:text-xs text-fg-secondary truncate block" title={font.family}>
          {font.family}
        </span>
      </td>
...
function FontRow({
  ...
  return (
    <>
      <tr {...devProps('FontRow')} className={`group border-b border-border-secondary hover:bg-bg-secondary transition-colors ${
        ...
        <td className="py-2 px-2 sm:px-3 w-[100px] sm:w-[140px] hidden sm:table-cell">
          {isEditing && editValues ? (
            <input
              ...
              disabled={EDITING_DISABLED}
              className="w-full px-2 py-1 text-xs rounded border border-border-primary bg-bg-primary text-fg-primary focus:border-border-brand focus:outline-hidden disabled:opacity-50 disabled:cursor-not-allowed"
            />
          ) : (
            <span className="text-[10px] sm:text-xs text-fg-secondary truncate block" title={fontFamily}>
              {fontFamily}
            </span>
          )}
        </td>
...
function AddFontRow({ onAdd, onCancel, isAdding }: AddFontRowProps) {
  ...
  return (
    <tr {...devProps('AddFontRow')} className="border-b border-border-brand bg-bg-brand-solid/10">
      {/* File Upload */}
      <td className="py-2 sm:py-3 px-2 sm:px-3 w-[50px] sm:w-[60px]">
        ...
        >
          {file ? (
            <Type01 className="w-4 h-4 sm:w-5 sm:h-5 text-fg-brand-primary" />
          ) : (
            <Upload01 className="w-3 h-3 sm:w-4 sm:h-4 text-fg-tertiary" />
          )}
          <input
            ref={fileInputRef}
            type="file"
      ...
      <td className="py-2 sm:py-3 px-2 sm:px-3 w-[100px] sm:w-[140px] hidden sm:table-cell">
        <input
          ...
          className="w-full px-2 py-1 text-xs rounded border border-border-brand bg-bg-primary text-fg-primary focus:outline-hidden"
        />
      </td>
      ...
      {isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-8 lg:p-12 pointer-events-none">
            <motion.div
              {...devProps('TypographySettingsTableModal')}
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              transition={{ duration: 0.2 }}
              className="w-full max-w-3xl max-h-[65vh] overflow-hidden rounded-2xl bg-bg-primary border border-border-primary shadow-2xl flex flex-col pointer-events-auto"
            >
              {/* Header */}
              ...
              <div className="flex-1 overflow-auto min-h-0">
                {isLoading ? (
                  <div className="flex items-center justify-center py-16 gap-3">
                    ...
                    <span className="text-fg-tertiary">Loading fonts...</span>
                  </div>
                ) : (
                    ...
                    <table className="w-full min-w-[480px]">
                      <thead className="sticky top-0 bg-bg-primary border-b border-border-secondary z-10">
                        <tr>
                          ...
                          <th className="py-2.5 px-2 sm:px-3 text-left text-[10px] font-semibold text-fg-tertiary uppercase tracking-wider w-[100px] sm:w-[140px] md:w-[180px]">
                            Name
                          </th>
                          <th className="py-2.5 px-2 sm:px-3 text-left text-[10px] font-semibold text-fg-tertiary uppercase tracking-wider w-[100px] sm:w-[140px] hidden sm:table-cell">
                            Family
                          </th>
                          <th className="py-2.5 px-2 sm:px-3 text-left text-[10px] font-semibold text-fg-tertiary uppercase tracking-wider w-[80px] sm:w-[100px] hidden sm:table-cell">
                            Weight
                          </th>
                          <th className="py-2.5 px-2 sm:px-3 text-left text-[10px] font-semibold text-fg-tertiary uppercase tracking-wider w-[70px] sm:w-[90px]">
                            Usage
                          </th>
                          ...
                          <th className="py-2.5 px-2 sm:px-3 text-right text-[10px] font-semibold text-fg-tertiary uppercase tracking-wider w-[50px] sm:w-[60px]">
              ...
              {/* Footer info */}
              <div className="p-3 sm:p-4 border-t border-border-primary bg-bg-secondary shrink-0">
