...
// Note: The canonical source for STATIC_SYSTEM_LOGOS is now
// LogoSettingsContent.tsx. This file retains its own copy
...
function CustomSelect({ 
  ...
  return (
    <div {...devProps('CustomSelect')} ref={containerRef} className="relative">
      <button
        ref={buttonRef}
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center gap-1.5 px-2 py-1 text-xs rounded-md border border-border-primary bg-bg-secondary hover:bg-bg-tertiary transition-colors min-w-[80px]"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute left-0 z-[100] py-1 min-w-[160px] max-h-[200px] overflow-auto rounded-lg border border-border-primary bg-bg-primary shadow-xl ${
...
function LogoRow({ logo, onDownload }: LogoRowProps) {
  ...
  return (
    <>
      <tr {...devProps('LogoRow')} className="group border-b border-border-secondary hover:bg-bg-tertiary transition-colors">
        {/* Preview */}
        <td className="py-2 px-2 sm:px-3 w-[50px] sm:w-[60px]">
...
function AddLogoRow({ onAdd, onCancel, isAdding, availableTypes, availableVariants }: AddLogoRowProps) {
  ...
  return (
    <tr {...devProps('AddLogoRow')} className="border-b border-border-brand bg-bg-brand-solid/10">
      {/* File Upload */}
      ...
      <td className="py-2 sm:py-3 px-2 sm:px-3 w-[50px] sm:w-[60px]">
        <div className="flex items-center justify-end gap-0.5 sm:gap-1">
          <button
            ...
            className="p-1 sm:p-1.5 rounded-lg border border-fg-brand-primary text-fg-brand-primary hover:bg-bg-brand-solid hover:text-white hover:border-bg-brand-solid transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            title="Add logo"
          >
            {isAdding ? (
      ...
      {isOpen && (
        <>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-8 lg:p-12 pointer-events-none">
            <motion.div
              {...devProps('LogoSettingsTableModal')}
                  ...
                  <table className="w-full min-w-[480px]">
                    <thead className="sticky top-0 bg-bg-primary border-b border-border-secondary z-10">
                      <tr>
                        ...
                        <th className="py-2.5 px-2 sm:px-3 text-left text-[10px] font-semibold text-fg-tertiary uppercase tracking-wider w-[80px] sm:w-[100px]">
                          Category
                        </th>
                        <th className="py-2.5 px-2 sm:px-3 text-left text-[10px] font-semibold text-fg-tertiary uppercase tracking-wider w-[80px] sm:w-[100px]">
                          Type
